...
class GUI:
    def __init__(self, opt):
        ...
        if self.opt.front_prompt is not None: 
            self.front_prompt = self.opt.front_prompt
        if self.opt.right_prompt is not None:
            self.right_prompt = self.opt.right_prompt
        if self.opt.left_prompt is not None:
            self.left_prompt = self.opt.left_prompt
        if self.opt.back_prompt is not None:
            self.back_prompt = self.opt.back_prompt
        if self.opt.right_front_prompt is not None:
            self.right_front_prompt = self.opt.right_front_prompt
        if self.opt.left_front_prompt is not None:
            self.left_front_prompt = self.opt.left_front_prompt
        if self.opt.right_back_prompt is not None:
            self.right_back_prompt = self.opt.right_back_prompt
        if self.opt.left_back_prompt is not None:
            self.left_back_prompt = self.opt.left_back_prompt

        print(f"[INFO] prompt: {self.prompt}")
        print(f"[INFO] negative prompt: {self.negative_prompt}")
        print(f"[INFO] front prompt: {self.front_prompt}")
        print(f"[INFO] right prompt: {self.right_prompt}")
        print(f"[INFO] left prompt: {self.left_prompt}")
        print(f"[INFO] back prompt: {self.back_prompt}")
        print(f"[INFO] right front prompt: {self.right_front_prompt}")
        print(f"[INFO] left front prompt: {self.left_front_prompt}")
        print(f"[INFO] right back prompt: {self.right_back_prompt}")
        print(f"[INFO] left back prompt: {self.left_back_prompt}")

        try:
            revert_y = self.opt.revert_y
        except:
            revert_y = False
        ...
        if self.opt.load is not None:
            self.renderer.initialize(self.opt.load, revert_y=revert_y)            
        else:
    ...
    def prepare_train(self):
        ...
        if self.guidance_sd is None and self.enable_sd:
            if self.opt.mvdream:
                ...
                print(f"[INFO] loading SD3...")
                from guidance.sd_utils import StableDiffusion3
                self.guidance_sd = StableDiffusion3(self.device, cache_dir=self.opt.cache_dir)
                print(f"[INFO] loaded SD3!")
            else:
                print(f"[INFO] loading SD...")
                from guidance.sd_utils import StableDiffusion
                # self.guidance_sd = StableDiffusion(self.device)
                self.guidance_sd = StableDiffusion(self.device, sds_bridge=self.opt.sds_bridge, cache_dir=self.opt.cache_dir)
                ...
                if self.opt.imagedream:
                    ...
                    if self.opt.mvsds:
                        prompts = {
                            ...
                            'right': self.right_prompt,
                            'left': self.left_prompt,
                            'back': self.back_prompt,
                            'right_front': self.right_front_prompt,
                            'left_front': self.left_front_prompt,
                            'right_back': self.right_back_prompt,
                            'left_back': self.left_back_prompt,
                        }
                        self.guidance_sd.get_text_embeds_multiview(prompts, [self.negative_prompt])
                    else:
    ...
    def train_step(self, cur_iter):
        ...
        for _ in range(self.train_steps):
            ...
            if self.opt.mvsds:
                cur_angle = self.opt.angle if cur_iter % 2 == 0 else 0
                cur_angles = [cur_angle, 180, -90]
                for i, hor_ini in enumerate(cur_angles):
                    ver = 0
                    # ver = np.random.randint(min_ver, max_ver)
                    hor_offset = np.random.randint(-10, 10)
                    hor = hor_ini + hor_offset
                    # hor = hor_ini
                    radius = 0
                    vers.append(ver)
                    hors.append(hor)
            ...
            if self.enable_sd:
                if self.opt.mvdream or self.opt.imagedream:
                    ...
                    if self.opt.mvsds:
                        # given hors, do view-dependent SDS update
                        loss = loss + self.opt.lambda_sd * self.guidance_sd.train_step(images, step_ratio=step_ratio if self.opt.anneal_timestep else None, hors=hors)
                    else:
    ...
    @torch.no_grad()
    def test_render(self, num_views=100):
        # render 360 views
        hors = np.linspace(-180, 180, num_views)
        outs = []
        for hor in hors:
            ...
            out = self.renderer.render(cur_cam, bg_color=bg_color)
            out = torch.permute(out["image"], (1, 2, 0))
            outs.append(out)        

        os.makedirs(self.opt.outdir, exist_ok=True)
        path = os.path.join(self.opt.outdir, self.opt.save_path + '_gs_360.mp4')
        writer = imageio.get_writer(path, fps=30)
        for frame in outs:
            writer.append_data((frame * 255).cpu().numpy().astype(np.uint8))
        writer.close()
...
if __name__ == "__main__":
    ...
    if opt.gui:
        gui.render()
    elif opt.test is not None and opt.test:
        gui.test_render()
    else:
        gui.train(opt.iters)
