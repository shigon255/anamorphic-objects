...
class GaussianModel:
    ...
    def load_ply(self, path, revert_y=False):
        ...
        if revert_y:
            # compatible with the opencv coordinate system
            xyz[:, 0] = -xyz[:, 0]
            xyz[:, 1] = -xyz[:, 1]
        ...
        extra_f_names = extra_f_names[:3*(self.max_sh_degree + 1) ** 2 - 3]
...
class Renderer:
    ...
    def initialize(self, input=None, num_pts=5000, radius=0.5, revert_y=False):
        # load checkpoint
        if input is None:
            ...
            # load from saved ply
            self.gaussians.load_ply(input, revert_y=revert_y)
